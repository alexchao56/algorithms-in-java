...
/** The following is an implementation of a binary search tree (BST). 
 * It is a node-based binary tree data structure with the folowing properties:
...
public class BinarySearchTree<T extends Comparable<T>> implements ITree<T> {
	...
	protected static final Random RANODM = new Random();
    ...
    /**Adds value to the tree and returns the Node that was added.
     * Tree can contain multiple equal values. */
    ...
    /** Locate T in the tree. Returns the first reference of value in tree
    ...
    /** Get greatest node in sub-tree rooted at startingNode.
     * The search does not include startingNode in it's results.
     * Returns Node<T> which represents the greatest node in the startingNode
    ...
    /** Get least node in sub-tree rooted at startingNOde. The search
     * does not include startingNode in it's results.
     * Returns Node<T> which represents the least node in the
     * startingNode sub-tree or NULL if startingNode has no lesser children. */
     ...
     * Returns Node<T> which was removed from the tree. */
     ...
     * Returns nodeRemove Node<T> removed from the tree, it can be different
     ...
     * algorithm from the tree. Returns Node<T> which can be used to replace nodeToRemoved.
    ...
    /** Validate the node for all Binary Search Tree invariants. Returns True if the node is valid. */
     ...
     * Returns breath first search sorted array representing the tree. */
    public T[] getBFS() {
